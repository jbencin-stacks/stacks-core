...
pub mod address;
pub mod auth;
pub use auth::{SpendingConditionVerifyExt, TransactionAuthExt, TransactionSpendingConditionExt};
pub mod block;
pub mod boot;
pub mod db;
pub mod events;
...
// Auth-tree primitive enums live in `stacks-codec`. Re-exported here so
// existing call sites (`crate::chainstate::stacks::TransactionAnchorMode`,
// `SinglesigHashMode`, etc.) keep working. `AddressHashMode` is also
// re-exported here because downstream test modules pick it up via
// `use super::*` from this module.
pub use stacks_codec::address::AddressHashMode;
pub use stacks_codec::transaction::{
    MultisigHashMode, OrderIndependentMultisigHashMode, SinglesigHashMode, TransactionAnchorMode,
    TransactionAuthField, TransactionAuthFieldID, TransactionAuthFlags,
    TransactionPublicKeyEncoding,
};
// Spending-condition types and `TransactionAuth` live in stacks-codec;
// re-exported alongside the auth-tree primitive enums above.
pub use stacks_codec::transaction::{
    MultisigSpendingCondition, OrderIndependentMultisigSpendingCondition,
    SinglesigSpendingCondition, TransactionAuth, TransactionSpendingCondition,
};
