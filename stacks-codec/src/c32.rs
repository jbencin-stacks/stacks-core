...
/// Error variants used by the c32 encode/decode helpers. Mirrors the relevant
/// subset of `stacks_common::address::Error`; stacks-common adds a
/// `From<Error>` impl on its full `address::Error` for back-compat.
#[derive(Debug)]
pub enum Error {
    InvalidCrockford32,
    InvalidVersion(u8),
...
pub fn c32_encode(input_bytes: &[u8]) -> String {
...
pub fn c32_decode(input_str: &str) -> Result<Vec<u8>, Error> {
    // must be ASCII
