...
use sha3::Keccak256;
// `Hash160` and `Sha512Trunc256Sum` appear inside the `StacksTransaction`
// type tree, so they live in `stacks-codec` (along with the `Hash20` /
// `Hash32` / `Hash64` serde helpers used in their `serialize_with` /
// `deserialize_with` attributes). Re-export so existing call sites keep
// working.
pub use stacks_codec::hash::{
    Hash160, Hash20, Hash32, Hash64, Sha512Trunc256Sum, HASH160_ENCODED_SIZE,
};
...
impl MerkleHashFunc for Sha512Trunc256Sum {
    ...
    fn from_tagged_data(tag: u8, data: &[u8]) -> Sha512Trunc256Sum {
        let mut sha2 = sha2::Sha512_256::new();
        sha2.update([tag]);
        sha2.update(data);
