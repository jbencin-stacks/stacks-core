...
// sites (`stacks_common::types::StacksPublicKeyBuffer`) keep working. The
// `from_public_key` / `to_public_key` conversion methods (previously the
// `StacksPublicKeyBufferExt` trait) are inherent on the type now that
// `Secp256k1PublicKey` lives alongside it in stacks-codec.
// `PublicKey` and `PrivateKey` traits live in stacks-codec (they only depend
// on `MessageSignature` which is also there). Re-exported so existing call
// sites (`stacks_common::types::{PublicKey, PrivateKey}`) keep working.
pub use stacks_codec::keys::{PrivateKey, PublicKey};
